...
export const create = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const { _id, _creationTime, photoStorageId: _photoStorageId, ...projectData } = project;
...
export const restore = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    for (const phaseSnapshot of versionRecord.phases) {
      const { steps, _id, _creationTime, projectId, ...phaseData } = phaseSnapshot;
      ...
      for (const stepSnapshot of steps) {
        const { _id, _creationTime, phaseId, projectId, ...stepData } = stepSnapshot;
...
export const getNextVersion = query({
  ...
  handler: async (ctx, args) => {
    ...
    if (!(organization && organization.autoVersioning)) {
      return null;
    }
