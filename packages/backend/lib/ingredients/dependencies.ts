...
export function findProjectIdsUsingIngredientCode<ProjectId extends string = string>(args: {
  ingredientCode?: string;
  ...
  const linkedInventoryIds = new Set(
    inventoryItems.filter((item) => item.ingredientCode === normalizedCode).map((item) => item._id),
  );
  ...
  return [
    ...new Set(
      recipeSteps
        .filter((step) => step.ingredientId && linkedInventoryIds.has(step.ingredientId))
